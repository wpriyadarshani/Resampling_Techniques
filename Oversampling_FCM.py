...
class fcm(object):
    ...
    def __init__(self, k=3, max_PCA_iterations=10, min_distance=5.0, size=200, m=2.5, epsilon=.5, max_FCM_iterations=100):
    ...
    def run(self, image):
        ...
        for j in range(80):
            for i in range(len(self.oversample_array)):
                self.pixels.append(numpy.asarray(self.oversample_array[i]))


        # set the size
        self.s_new = self.s + ( 80 * len(self.oversample_array))
        ...
        for i in range(200):
            for j in range(200):
                if self.calcDistance(GT_pixelmap[i,j], pixels_GT[j1]) == 0.0: 
                    ...
                    self.oversample_array.append(pixel)

               
        print "length of oversample " , len(self.oversample_array)
    ...
    def getVariance(self, cluster):
        ...
        for i in range(self.s):
            if self.degree_of_membership[i][cluster] == 1:
                dis = self.calcDistance(self.pixels[i], cluster)
